/* eslint-disable */
// This is a copy of `OPFSAnyContextVFS` in `wa-sqlite@1.1.0`,
 ...
 * @param {FileSystemDirectoryHandle} rootDirectoryHandle
 * @returns {Promise<[FileSystemDirectoryHandle, string]>}
 */
async function getPathComponents(pathname, create, rootDirectoryHandle) {
...
export class OPFSAnyContextVFS extends WebLocksMixin(FacadeVFS) {
  ...
  async getPathComponents(path, create) {
    ...
    return getPathComponents(pathname, create, this.releaser)
  }

  static async create(name, module, options, rootDirectoryHandle) {
    const vfs = new OPFSAnyContextVFS(name, module, options)
    vfs.releaser = rootDirectoryHandle ?? await navigator.storage.getDirectory()
    await vfs.isReady()
    return vfs
  }
  ...
  async jOpen(zName, fileId, flags, pOutFlags) {
    try {
      ...
      const [directoryHandle, filename] = await getPathComponents(pathname, create, this.releaser)
  ...
  async jDelete(zName, syncDir) {
    try {
      ...
      const [directoryHandle, name] = await getPathComponents(pathname, false, this.releaser)
  ...
  async jAccess(zName, flags, pResOut) {
    try {
      ...
      const [directoryHandle, dbName] = await getPathComponents(pathname, false, this.releaser)
  ...
  async jClose(fileId) {
    try {
      ...
      if (file?.flags & VFS.SQLITE_OPEN_DELETEONCLOSE) {
        const [directoryHandle, name] = await getPathComponents(file.pathname, false, this.releaser);
