...
import { initSQLite, isModuleWorkerSupport, isOpfsSupported } from '@subframe7536/sqlite-wasm'
...
initSQLite(useIdbStorage('test', {
  ...
  .then(async ({ run }) => {
    console.log(await isOpfsSupported())
    console.log(await isModuleWorkerSupport())
